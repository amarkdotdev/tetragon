...
func setupResolvePathInContainer(sensor *sensors.Sensor, spec *v1alpha1.TracingPolicySpec, polInfo *policyInfo) {
	...
	// Disable runs preUnload before the manager's load lock; PreUnload is the
	// idempotent fallback for delete and load errors. PreUnload also fires from
	// collection.load()'s rollback under muLoad — safe only because
	// "generic_uprobe" sorts last in its collection (see sortSensors), so no
	// sibling can fail after this one loads. Keep that ordering.
...
// policyKey is the registry key for a policy. It carries the same triple as
// the sensor manager's collection identity (name, namespace, domain) so that
// policies sharing a name across namespaces or domains (e.g. k8s vs static)
// cannot collide; the formatting is internal to this registry.
...
func registerResolvePathInContainer(spec *v1alpha1.TracingPolicySpec, polInfo *policyInfo) (postLoad, preUnload sensors.SensorHook) {
	...
	// activeRec is the loaded generation's reconciler, or nil once torn down.
...
func reconcileExistingContainers(snapshot reconcilerSnapshot, pods []*corev1.Pod, running []cri.RunningContainer) (matched, applied int) {
	...
	// Matching depends only on the pod, so evaluate each policy's selector once
	// per pod rather than once per (container, policy).
	podPolicies := make(map[string][]bool, len(current))
	for uid, pod := range current {
		policies := make([]bool, len(snapshot))
		for i, entry := range snapshot {
			policies[i] = entry.match(pod.namespace, pod.labels)
		}
		podPolicies[uid] = policies
	}
	for _, container := range running {
		...
		key := containerKey(container.PodUID, container.ID)
		for i, entry := range snapshot {
			if podPolicies[container.PodUID][i] {
				desired[entry.r][key] = struct{}{}
			}
		}
	}
	for _, entry := range snapshot {
