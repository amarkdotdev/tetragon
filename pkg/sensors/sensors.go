...
// chainHooks composes two sensor hooks that must fail fast: next does not run
// if prev fails.
func chainHooks(prev, next SensorHook) SensorHook {
	if prev == nil {
		return next
	}
	return func() error {
...
func (s *Sensor) AddPostMapLoadHook(hook SensorHook) {
	s.PostMapLoadHook = chainHooks(s.PostMapLoadHook, hook)
}
...
// policies are single-section: the only siblings are "__enforcer__" (sorted
// first) and "__loader__", both before "generic_uprobe";
// TestSortSensorsGenericUprobeLast guards those two. Note "generic_usdt" would
// sort after "generic_uprobe" — only the single-section policy validation
// keeps them out of one collection, so lifting that restriction means
// revisiting this ordering.
